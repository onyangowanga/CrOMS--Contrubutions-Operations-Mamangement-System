...
export async function generateWhatsappSummary(campaignId: string): Promise<string> {
  const campaignResult = await pool.query(
    "SELECT id, name, target_amount, total_raised FROM campaigns WHERE id = $1",
    [campaignId]
  );
  ...
  const contribResult = await pool.query(
    "SELECT display_name, total_contributed FROM contributors WHERE campaign_id = $1 AND total_contributed > 0 ORDER BY total_contributed DESC",
    [campaignId]
  );
  ...
  const contributorLines = (contribResult.rows as Array<{ display_name: string; total_contributed: string }>).length
    ? (contribResult.rows as Array<{ display_name: string; total_contributed: string }>)
        .map((c, i) => `${i + 1}. ${c.display_name} - KES ${currency(Number(c.total_contributed))}`)
        .join("\n")
  ...
  return [
    ...
    targetAmount === null ? "TARGET: Not set" : `TARGET: KES ${currency(targetAmount)}`,
    deficit === null ? "DEFICIT: N/A" : `DEFICIT: KES ${currency(deficit)}`,
    "",
